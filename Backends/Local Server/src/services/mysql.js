...
const sql = {
	...
	tables: {
		...
		rfidCard:'CREATE TABLE IF NOT EXISTS RFID_Card(CardId VARCHAR(20) PRIMARY KEY, EmployeeId VARCHAR(12) , CustomerName VARCHAR(25), Date DATE , Time TIME ,SecurityTag VARCHAR(20) ,IsIssued BOOLEAN DEFAULT false, Amount DOUBLE, FOREIGN KEY (EmployeeId) REFERENCES EMPLOYEE (NIC) ON UPDATE CASCADE)',
		...
		issueLog:'CREATE TABLE IF NOT EXISTS ISSUE_LOG(LogId INT PRIMARY KEY AUTO_INCREMENT,NIC VARCHAR(12),CardId VARCHAR(20),CustomerName VARCHAR(30),DepositAmount DOUBLE,Date DATE , Time TIME )'
	}
}
